...
def gptu_check(image, bot_id=bot_id, url=url, headers=headers):
    prompt = "Hãy đọc và phân tích bảng điểm trong đường link sau: http://localhost:8000" + \
        str(image)+" . Sau khi đọc, bạn hãy trả về điểm mạnh, điểm yếu, những ngành nghề phù hợp, những môn thi phù hợp, những trường đại học phù hợp với bảng điểm đó. Hãy trả lại câu trả lời ở dạng code HTML và thêm một chút CSS để cách dòng, đánh dấu, in đậm, highlight. Hãy chỉ trả về đoạn code mà không trả về bất cứ thứ gì khác như lưu ý về đoạn code, giải thích đoạn code, ..."
    data = {
        "conversation_id": "123",
        "bot_id": bot_id,
        "user": "123333333",
