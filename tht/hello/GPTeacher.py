...
def search_image(image, bot_id=bot_id, url=url, headers=headers):
    prompt = "Hãy đọc và trả lời câu hỏi trong bức ảnh trong đường link: http://localhost:8000" + \
        str(image) + " . Hãy trả lại câu trả lời dưới dạng code HTML và thêm một chút CSS để in đậm, cách dòng, highlight. Hãy chỉ trả về đoạn code và không trả về lưu ý về đoạn code, giải thích hay những thứ khác không phải là đoạn code"
    data = {
        "conversation_id": "123",
        "bot_id": bot_id,
        "user": "123333333",
